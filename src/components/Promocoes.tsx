...
import { WHATSAPP_RESERVA_URL } from '../constants'
...
export default function Promocoes() {
  ...
  useEffect(() => {
    fetch('/api/cardapio')
      .then((r) => r.json())
      .then((data: { name: string; description: string; category: string }[]) => {
        if (!Array.isArray(data)) return
        const cfg: Partial<RodizioConfig> = {}
        data
          .filter((item) => item.category === 'RODIZIO_CONFIG')
          .forEach((item) => {
            const key = item.name as keyof RodizioConfig
            cfg[key] = item.description
          })
        if (Object.keys(cfg).length > 0) setConfig({ ...FALLBACK, ...cfg })
      })
      .catch(() => {/* mantém fallback */})
  }, [])

  const [reais, centavos] = config.Preco.replace('R$', '').trim().split(',')
    ...
    <section id="promocoes" className="relative py-0 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-24 grid md:grid-cols-2 gap-12 items-center">
        ...
        <div className="reveal text-white">
          ...
          <h2 className="font-display text-7xl md:text-8xl leading-none mt-2 mb-1">Rodízio</h2>
          <h2 className="font-display text-5xl md:text-6xl leading-none text-brand-yellow mb-8">de Pizza</h2>

          <ul className="space-y-3 mb-10 font-body">
            <li className="flex items-center gap-3 text-lg">
              <span className="text-brand-yellow text-xl">📅</span>
              <span><strong className="font-heading">{config.Dias}</strong> — toda semana</span>
            </li>
            <li className="flex items-center gap-3 text-lg">
              <span className="text-brand-yellow text-xl">⏰</span>
              <span><strong className="font-heading">{config.Horario}</strong></span>
            </li>
            <li className="flex items-center gap-3 text-lg">
              <span className="text-brand-yellow text-xl">🎂</span>
        ...
        <div className="reveal flex justify-center md:justify-end">
          <div className="bg-white/10 backdrop-blur-sm border border-white/20 text-white text-center px-12 py-14 shadow-2xl max-w-xs w-full">
            ...
            <div className="font-display leading-none">
              ...
              <span className="text-8xl text-brand-yellow">{reais}</span>
              <span className="text-4xl">,{centavos ?? '90'}</span>
            </div>
            <span className="font-heading text-sm uppercase tracking-widest text-white/70 block mt-3">
              por pessoa
            </span>
            <div className="mt-8 pt-6 border-t border-white/20">
              <p className="font-body text-sm text-white/70 leading-relaxed">{config.Descricao}</p>
            </div>
          </div>
        </div>
      </div>
